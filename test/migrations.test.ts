...
describe('numbered migration rollout',()=>{
  it('applies primary 0001 then 0002 then 0003 to an empty database',()=>{const db=join(dir,'primary.db');run(db,primary1);run(db,primary2);run(db,primary3);expect(JSON.parse(query(db,"SELECT value FROM schema_meta WHERE key='schema_version'"))[0].value).toBe('3');expect(JSON.parse(query(db,"SELECT COUNT(*) count FROM sqlite_master WHERE type='trigger' AND name IN ('primary_snapshots_no_update','primary_snapshots_no_delete','primary_snapshot_entries_no_update','primary_snapshot_entries_no_delete')"))[0].count).toBe(4);expect(query(db,'PRAGMA foreign_key_check')).toBe('');});
