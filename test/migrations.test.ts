...
const primary5=readFileSync(new URL('../migrations/0005_personal_recovery_question.sql',import.meta.url),'utf8');
...
describe('numbered migration rollout',()=>{
  it('applies primary migrations to an empty database',()=>{const db=join(dir,'primary.db');run(db,primary1);run(db,primary2);run(db,primary3);run(db,primary4);run(db,primary5);expect(JSON.parse(query(db,"SELECT value FROM schema_meta WHERE key='schema_version'"))[0].value).toBe('5');expect(JSON.parse(query(db,"SELECT COUNT(*) count FROM sqlite_master WHERE type='trigger' AND name IN ('primary_snapshots_no_update','primary_snapshots_no_delete','primary_snapshot_entries_no_update','primary_snapshot_entries_no_delete')"))[0].count).toBe(4);expect(query(db,'PRAGMA foreign_key_check')).toBe('');});
