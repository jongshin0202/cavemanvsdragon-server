...
describe('numbered migration rollout',()=>{
  it('applies primary 0001 then 0002 to an empty database',()=>{const db=join(dir,'primary.db');run(db,primary1);run(db,primary2);expect(JSON.parse(query(db,"SELECT value FROM schema_meta WHERE key='schema_version'"))[0].value).toBe('2');});
  ...
  it('safely reapplies idempotent migration SQL',()=>{expect(primary2).toContain('CREATE TABLE IF NOT EXISTS');expect(backup2).toContain('INSERT OR IGNORE');expect(()=>run(join(dir,'primary.db'),primary2)).not.toThrow();expect(()=>run(join(dir,'backup.db'),backup2)).not.toThrow();});
