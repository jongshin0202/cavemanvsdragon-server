...
<main>
  ...
  <section class="panel" id="primaryLeaderboard">
    ...
    <div class="toolbar">
      ...
      <label><input id="showRemovedPrimary" type="checkbox"> Show removed entries</label>
  ...
  <section class="panel" id="primarySnapshots">
    ...
    <p>Immutable, timestamped snapshots include active and soft-deleted entries. “Remove from active list” preserves the recovery point and its read-only contents; snapshots are never permanently deleted.</p>
    <div class="toolbar"><button id="snapshots">Refresh snapshots</button><label><input id="showRemovedSnapshots" type="checkbox"> Show removed snapshots</label><button id="latestPreClear" class="safe">Restore latest pre-clear snapshot</button></div>
    <div id="snapshotList" class="table-wrap" hidden><table><thead><tr><th>Status</th><th>Created UTC</th><th>Trigger type</th><th>Actor</th><th>Entry count</th><th>Reason</th><th>Snapshot ID</th><th>Actions</th></tr></thead><tbody id="snapshotRows"></tbody></table></div>
  ...
  <section class="panel" id="backupLeaderboard">
    ...
    <div class="toolbar"><button id="backupList">Refresh/list backup leaderboard</button><label><input id="showRemovedBackup" type="checkbox"> Show removed entries</label><button id="backupSaveOrder">Save backup displayed order</button><button id="backupHistory">Backup admin history</button><button id="backupExport">Export backup leaderboard CSV</button><button id="backup">Backup status</button></div>
...
<script>
  ...
  async function loadLeaderboard(){
    try{setNotice('Loading…');var result=await api('/v1/admin/leaderboard?limit=500'+(document.getElementById('showRemovedPrimary').checked?'&include_deleted=true':''));displayed=result.data.entries;renderRows();setNotice('Loaded '+result.data.total+' entries.','success');}
  ...
  function renderSnapshots(){var list=document.getElementById('snapshotList'),el=document.getElementById('snapshotRows');el.textContent='';snapshotDisplayed.forEach(function(row){var tr=document.createElement('tr'),id=document.createElement('code'),actions=document.createElement('div');id.className='snapshot-id';id.textContent=row.id;actions.className='row-actions';actions.appendChild(button('Copy ID',function(){copySnapshotId(row.id);}));actions.appendChild(button('View contents',function(){viewSnapshotContents(row.id);}));if(row.archived){actions.appendChild(button('Unarchive',function(){unarchiveChosenSnapshot(row.id);},'safe'));}else{actions.appendChild(button('Restore this snapshot',function(){restoreChosenSnapshot(row.id);},'safe'));actions.appendChild(button('Remove from active list',function(){archiveChosenSnapshot(row.id);},'danger'));}[row.archived?'Archived':'Active',row.created_at,row.trigger_type,row.actor,row.entry_count,readable(row.reason)].forEach(function(value){tr.appendChild(cell(value));});tr.appendChild(cell(id));tr.appendChild(cell(actions));el.appendChild(tr);});list.hidden=false;}
  async function loadSnapshots(){try{var result=await api('/v1/admin/snapshots?limit=100'+(document.getElementById('showRemovedSnapshots').checked?'&include_archived=true':''));snapshotDisplayed=result.data;renderSnapshots();setNotice('Loaded '+snapshotDisplayed.length+' snapshots.','success');}catch(e){setNotice(e.message,'error');}}
  ...
  async function archiveChosenSnapshot(id){if(!window.confirm('Remove this snapshot from the active list? The recovery point and its contents remain preserved.'))return;try{var ch=await api('/v1/admin/snapshots/'+encodeURIComponent(id)+'/archive/challenge',{method:'POST',body:'{}'});if(!window.confirm('Final confirmation: remove snapshot '+id+' from the active list?'))return;var r=await api('/v1/admin/snapshots/'+encodeURIComponent(id)+'/archive',{method:'POST',body:JSON.stringify({challenge_id:ch.data.challenge_id,confirmation:true,second_confirmation:true,confirmation_phrase:ch.data.confirmation_phrase,reason:'Admin dashboard removal'})});setLastAudit(r.data.audit_id);setNotice('Snapshot removed from active list; recovery point preserved.','success');await loadSnapshots();}catch(e){setNotice(e.message,'error');}}
  async function unarchiveChosenSnapshot(id){if(!window.confirm('Unarchive snapshot '+id+' and restore it to the active list?'))return;try{var r=await api('/v1/admin/snapshots/'+encodeURIComponent(id)+'/unarchive',{method:'POST',body:JSON.stringify({confirmation:true,snapshot_id:id})});setLastAudit(r.data.audit_id);setNotice('Snapshot restored to the active list.','success');await loadSnapshots();}catch(e){setNotice(e.message,'error');}}
  ...
  async function loadBackupRows(){try{var r=await api('/v1/admin/backup-leaderboard?limit=500'+(document.getElementById('showRemovedBackup').checked?'&include_removed=true':''));backupDisplayed=r.data;renderBackupRows();setNotice('Loaded independent managed backup state.','success');}catch(e){setNotice(e.message,'error');}}
